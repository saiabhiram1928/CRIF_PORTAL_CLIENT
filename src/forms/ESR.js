...
const ESR = (props) => {
  ...
  let tariff = {
    int_stu: { Room: 200, Variable: 400, Low: 400 },
  ...
  const onSubmit = (data) => {
    ...
    processedData.esr_range = data.esr_range;
    ...
    props.setTotalPrice(props.totalPrice + tariff[userDetails.role][processedData.temperature])
    reset({ sample_code: "", solubility: "", esr_range: "" });
      ...
      <div className={styles["two-fields"]}>
        ...
        >
          <input
            ...
            style={{ width: "100%", height: "2.4vw" }}
            type="text"
            placeholder="Expected ESR Range"
            {...register("esr_range", {
              required: true,
            })}
          />
          <p className={gstyles["field-error"]}>
            {errors.esr_range?.type === "required" &&
              "Expected ESR Range is Required"}
          </p>
        </div>
      </div>
      <MarginBox margin="0.6vw" />
